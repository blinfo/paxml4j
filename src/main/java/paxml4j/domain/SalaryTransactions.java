...
public class SalaryTransactions implements Entity {
    ...
    public enum SalaryCode {
        ...
        public static SalaryCode find(String text) {
            return Stream.of(values()).filter(sc -> sc.name().equalsIgnoreCase(text) || sc.code().equalsIgnoreCase(text)).findFirst().orElseThrow(() -> new Paxml4jException("No SalaryCode found for code: " + text));
        }
    }
}
